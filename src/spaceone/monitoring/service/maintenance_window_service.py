...
class MaintenanceWindowService(BaseService):
    def __init__(self, *args, **kwargs):
        ...
        self.maintenance_window_mgr: MaintenanceWindowManager = (
            self.locator.get_manager("MaintenanceWindowManager")
        )
    ...
    def update(self, params):
        ...
        maintenance_window_vo = self.maintenance_window_mgr.get_maintenance_window(
            maintenance_window_id, domain_id
        )

        if maintenance_window_vo.state == "CLOSED":
            raise ERROR_CLOSED_MAINTENANCE_WINDOW(
        ...
        return self.maintenance_window_mgr.update_maintenance_window_by_vo(
            params, maintenance_window_vo
        )
    ...
    def close(self, params):
        ...
        return self.maintenance_window_mgr.close_maintenance_window(
            params["maintenance_window_id"], params["domain_id"]
        )
    ...
    def get(self, params):
        ...
        return self.maintenance_window_mgr.get_maintenance_window(
            params["maintenance_window_id"], params["domain_id"], params.get("only")
        )
    ...
    def close_maintenance_window(self, params):
        ...
        maintenance_window_vos: List[
            MaintenanceWindow
        ] = maintenance_window_mgr.list_open_maintenance_windows()
        ...
        for maintenance_window_vo in maintenance_window_vos:
            if current_time > maintenance_window_vo.end_time:
                _LOGGER.debug(
                    f"[close_maintenance_window] Close out of time maintenance window "
                    f"({maintenance_window_vo.maintenance_window_id}): "
                    f"Current Time({current_time}) > End Time({maintenance_window_vo.end_time})"
                )
                maintenance_window_mgr.update_maintenance_window_by_vo(
                    {"state": "CLOSED"}, maintenance_window_vo
                )
