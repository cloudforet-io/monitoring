...
class InventoryManager(BaseManager):
    def __init__(self, *args, **kwargs):
        ...
        self.inventory_connector: SpaceConnector = self.locator.get_connector(
            "SpaceConnector", service="inventory"
        )

    def get_server(self, server_id, domain_id):
        return self.inventory_connector.dispatch(
            "Server.get", {"server_id": server_id, "domain_id": domain_id}
        )
    ...
    def _make_query(resources, required_keys):
        only_keys = list(
            set(
                ["cloud_service_id", "collection_info.secret_id", "region_code"]
                + required_keys
            )
        )
        only_keys.sort()
        ...
        return {
            "filter": [{"k": "cloud_service_id", "v": resources, "o": "in"}],
            "only": only_keys,
        }
