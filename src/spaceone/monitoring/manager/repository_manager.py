...
class RepositoryManager(BaseManager):
    ...
    def get_plugin(self, plugin_id, domain_id):
        return self.repo_connector.dispatch('Project.get', {'plugin_id': plugin_id, 'domain_id': domain_id})

    def check_plugin_version(self, plugin_id, version, domain_id):
        response = self.repo_connector.dispatch('Project.get_versions', {'plugin_id': plugin_id, 'domain_id': domain_id})
