...
class AlertManager(BaseManager):
    ...
    def add_responder(self, params):
        ...
        for responder in alert_vo.responders:
            if (
                responder.resource_type == resource_type
                and responder.resource_id == resource_id
            ):
                raise ERROR_SAME_RESPONDER_ALREADY_EXISTS(
                    resource_type=resource_type, resource_id=resource_id
                )

        alert_vo = alert_vo.append(
            "responders", {"resource_type": resource_type, "resource_id": resource_id}
        )
        return alert_vo
    ...
    def remove_responder(self, params):
        ...
        for responder in alert_vo.responders:
            if (
                ...
                alert_vo = alert_vo.remove(
                    "responders",
                    {"resource_type": resource_type, "resource_id": resource_id},
                )
                return alert_vo

        raise ERROR_RESPONDER_NOT_EXIST(
            resource_type=resource_type, resource_id=resource_id
        )
    ...
    def add_project_dependency(self, params):
        ...
        for project_dependency in alert_vo.project_dependencies:
            if project_dependency == project_id:
                raise ERROR_SAME_PROJECT_DEPENDENCY_ALREADY_EXISTS(
    ...
    def _get_alert_number(self, domain_id):
        def _rollback(alert_number_vo: AlertNumber):
            _LOGGER.info(
                f"[_get_alert_number._rollback] Decrement Number: {alert_number_vo.next}"
            )
