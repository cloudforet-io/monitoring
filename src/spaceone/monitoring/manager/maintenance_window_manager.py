...
class MaintenanceWindowManager(BaseManager):
    def __init__(self, *args, **kwargs):
        ...
        self.maintenance_window_model: MaintenanceWindow = self.locator.get_model(
            "MaintenanceWindow"
        )
    ...
    def create_maintenance_window(self, params):
        def _rollback(maintenance_window_vo):
            _LOGGER.info(
                f"[create_maintenance_window._rollback] "
        ...
        maintenance_window_vo: MaintenanceWindow = self.maintenance_window_model.create(
            params
        )
    ...
    def update_maintenance_window(self, params):
        maintenance_window_vo: MaintenanceWindow = self.get_maintenance_window(
    ...
    def update_maintenance_window_by_vo(self, params, maintenance_window_vo):
        def _rollback(old_data):
            _LOGGER.info(
                f"[update_maintenance_window_by_vo._rollback] Revert Data : "
    ...
    def close_maintenance_window(self, maintenance_window_id, domain_id):
        ...
        return self.update_maintenance_window_by_vo(
            {"state": "CLOSED"}, maintenance_window_vo
        )

    def get_maintenance_window(self, maintenance_window_id, domain_id, only=None):
        return self.maintenance_window_model.get(
            maintenance_window_id=maintenance_window_id, domain_id=domain_id, only=only
        )
