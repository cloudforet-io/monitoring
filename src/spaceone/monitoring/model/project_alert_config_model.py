...
class AlertOptions(EmbeddedDocument):
    notification_urgency = StringField(
        max_length=20, default="ALL", choices=("ALL", "HIGH_ONLY")
    )
    recovery_mode = StringField(
        max_length=20, default="MANUAL", choices=("MANUAL", "AUTO")
    )
...
class ProjectAlertConfig(MongoModel):
    ...
    meta = {
        "updatable_fields": ["options", "escalation_policy", "escalation_policy_id"],
        "minimal_fields": ["project_id", "options"],
        "change_query_keys": {"user_projects": "project_id"},
        "reference_query_keys": {"escalation_policy": EscalationPolicy},
        "indexes": [
            # 'project_id',
            "escalation_policy",
