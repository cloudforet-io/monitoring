        ...
        info.update(
            {
                ...
                "created_at": utils.datetime_to_iso8601(
                    maintenance_window_vo.created_at
                ),
                "updated_at": utils.datetime_to_iso8601(
                    maintenance_window_vo.updated_at
                ),
...
def MaintenanceWindowsInfo(maintenance_window_vos, total_count, **kwargs):
    return maintenance_window_pb2.MaintenanceWindowsInfo(
        results=list(
            map(
                functools.partial(MaintenanceWindowInfo, **kwargs),
