...
class TestDataSourceAPI(unittest.TestCase):
    ...
    def test_register_data_source(self, mock_parse_request, *args):
        ...
        data_source_data = MessageToDict(
            data_source_info, preserving_proto_field_name=True
        )
        self.assertIsInstance(data_source_info, data_source_pb2.DataSourceInfo)
        self.assertEqual(data_source_info.name, params["name"])
        self.assertEqual(
            data_source_info.state, data_source_pb2.DataSourceInfo.State.ENABLED
        )
        self.assertEqual(
            data_source_info.monitoring_type, data_source_pb2.MonitoringType.METRIC
        )
        ...
        self.assertIsInstance(
            data_source_info.plugin_info, data_source_pb2.DataSourcePluginInfo
        )
        self.assertEqual(
            data_source_data["plugin_info"]["plugin_id"],
            params["plugin_info"]["plugin_id"],
        )
        self.assertEqual(
            data_source_data["plugin_info"]["version"], params["plugin_info"]["version"]
        )
        self.assertEqual(
            data_source_data["plugin_info"]["secret_id"],
    ...
    def test_list_data_sources(self, mock_parse_request, *args):
        ...
        self.assertIsInstance(
            data_sources_info.results[0], data_source_pb2.DataSourceInfo
        )
